...
## Two-node BAGEL validation

The repository provides a two-node smoke benchmark for the RDMA path:

```text
tests/distributed/omni_connectors/run_bagel_yuanrong_two_node.py
```

Run it on the stage-0 machine. The script starts stage 1 through SSH, creates
per-node deploy YAML files under `/tmp/vllm-omni-bagel-cross-node`, sends three
concurrent image requests, and cleans up the process groups it started.

Before running, verify all of the following:

- The BAGEL model directory exists at the same path on both nodes, or pass the
  path with `--model`.
- The stage-0 machine can run `vllm serve` and can SSH to stage 1 with
  `BatchMode=yes`.
- The repository and Python environment are available at the same absolute
  path on both nodes. The remote virtual environment defaults to
  `/app/vllm-omni/.venv`; override it with `VLLM_OMNI_REMOTE_VENV`.
- The two `--stage*-ip` values are routable RDMA/ZMQ addresses and the chosen
  ports are available.
- The Yuanrong TransferEngine Python binding is installed in both runtime
  environments. CUDA mode additionally requires a GPU-enabled Yuanrong build
  and a working GPUDirect RDMA setup.

CPU memory-pool validation:
...
CUDA GPUDirect RDMA validation:
...
Use `--memory-pool-device cuda:N` only when the local CUDA ordinal is explicitly
known. For Yuanrong, do not pass an HCA name through `--rdma-device`: Yuanrong
uses `device_name: auto` and resolves the endpoint to the same CUDA or CPU
device as the memory pool. `--rdma-device` is retained for Mooncake runs.

Useful artifacts are `stage0.log`, `stage1.log`, `bench.log`, and
`controller.log` in the run directory. A successful run should show a ready
stage-0 health endpoint, successful benchmark completion, and receiver-side
Yuanrong transfer messages. Sender-side success alone does not prove that the
receiver completed the RDMA read.
