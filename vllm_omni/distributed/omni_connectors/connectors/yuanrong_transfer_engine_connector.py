...
def _requires_ascend_runtime(protocol: str, pool_device: str) -> bool:
    protocol = str(protocol).strip().lower()
    pool_device = str(pool_device).strip().lower()
    return protocol in {"ascend", "hccl", "npu"} or pool_device == "npu" or pool_device.startswith("npu:")


def _ensure_ascend_runtime_available() -> None:
    if find_spec("vllm_ascend") is None:
        raise ImportError(
            "YuanrongTransferEngineConnector Ascend/NPU mode requires vllm_ascend. "
            "Use protocol='rdma' with a CPU memory pool for CPU/GPU RDMA paths, "
            "or install vllm_ascend before using protocol='ascend'."
        )
